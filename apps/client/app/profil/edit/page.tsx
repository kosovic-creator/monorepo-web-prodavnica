...
import { getKorisnikById, updateProfilKorisnika, } from '@actions/korisnici';
import {updatePodaciPreuzimanja, createPodaciPreuzimanja } from '@actions/podaci-preuzimanja';
...
export default function EditProfilPage() {
  ...
  useEffect(() => {
    ...
    const loadKorisnik = async () => {
      try {
        ...
        // Provide default podaciPreuzimanja if missing
        const podaciPreuzimanja = (userData as any).podaciPreuzimanja || {
          telefon: '',
          drzava: '',
          grad: '',
          postanskiBroj: '',
        ...
        setForm({
          ...
          drzava: podaciPreuzimanja.drzava || '',
          grad: podaciPreuzimanja.grad || '',
          postanskiBroj: podaciPreuzimanja.postanskiBroj ? podaciPreuzimanja.postanskiBroj.toString() : '',
          adresa: podaciPreuzimanja.adresa || '',
          uloga: userData.uloga || 'korisnik',
          podaciId: podaciPreuzimanja.id || '',
        });
      } catch (error) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      ...
      result.error.issues.forEach((err: { path: any[]; message: string; }) => {
